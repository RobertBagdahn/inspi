...
def register_detail_attribute(request, reg_id):
    ...
    for module in event_modules:
        ...
        for attr_module in attribute_modules:
            ...
            elif attr_module.field_type == "DaA":
                try:
                    date_attr = DateAttribute.objects.get(
                        attribute_module=attr_module, registration=register
                    )
                    attribute_data["value"] = date_attr.date_field
                    attribute_data["id"] = date_attr.id
                except DateAttribute.DoesNotExist:
                    pass
            elif attr_module.field_type == "HTA":
                try:
                    html_attr = HTMLAttribute.objects.get(
                        attribute_module=attr_module, registration=register
                    )
                    attribute_data["value"] = html_attr.html_field
                    attribute_data["id"] = html_attr.id
                except HTMLAttribute.DoesNotExist:
                    pass
            elif attr_module.field_type == "RaA":
                try:
                    radio_attr = RadioAttribute.objects.get(
                        attribute_module=attr_module, registration=register
                    )
                    attribute_data["value"] = (
                        radio_attr.selected_option.text
                        if radio_attr.selected_option
                        else None
                    )
                    attribute_data["id"] = radio_attr.id
                    # Include all available options for this radio attribute
                    attribute_data["options"] = []
                    for option in AttributeChoiceOption.objects.filter(
                        attribute_module=attr_module
                    ):
                        attribute_data["options"].append(
                            {
                                "id": option.id,
                                "text": option.text,
                                "selected": (
                                    option.id == radio_attr.selected_option.id
                                    if radio_attr.selected_option
                                    else False
                                ),
                            }
                        )
                except RadioAttribute.DoesNotExist:
                    pass
            elif attr_module.field_type == "MuA":
                try:
                    multi_select_attr = MultiSelectAttribute.objects.get(
                        attribute_module=attr_module, registration=register
                    )
                    attribute_data["value"] = [
                        option.text
                        for option in multi_select_attr.selected_options.all()
                    ]
                    attribute_data["id"] = multi_select_attr.id
                except MultiSelectAttribute.DoesNotExist:
                    pass
            elif attr_module.field_type == "ZiA":
                try:
                    zip_code_attr = ZipCodeAttribute.objects.get(
                        attribute_module=attr_module, registration=register
                    )
                    attribute_data["value"] = zip_code_attr.zip_code_field.zip_code
                    attribute_data["id"] = zip_code_attr.id
                except ZipCodeAttribute.DoesNotExist:
                    pass
            elif attr_module.field_type == "EmA":
                try:
                    email_attr = EmailAttribute.objects.get(
                        attribute_module=attr_module, registration=register
                    )
                    attribute_data["value"] = email_attr.email_field
                    attribute_data["id"] = email_attr.id
                except EmailAttribute.DoesNotExist:
                    pass
            elif attr_module.field_type == "PhA":
                try:
                    phone_attr = PhoneAttribute.objects.get(
                        attribute_module=attr_module, registration=register
                    )
                    attribute_data["value"] = phone_attr.phone_number_field
                    attribute_data["id"] = phone_attr.id
                except PhoneAttribute.DoesNotExist:
                    pass
            elif attr_module.field_type == "ScA":
                try:
                    scout_group_attr = ScoutGroupAttribute.objects.get(
                        attribute_module=attr_module, registration=register
                    )
                    attribute_data["value"] = scout_group_attr.scout_group.name
                    attribute_data["id"] = scout_group_attr.id
                except ScoutGroupAttribute.DoesNotExist:
                    pass
            else:
                raise ValueError(f"Unknown attribute type: {attr_module.field_type}")
...
class RegistrationWizardView(SessionWizardView):
    ...
    def get_form_list(self):
        ...
        for i, event_module in enumerate(event_modules):
            ...
            form_list[f"module-{str(i)}"] = form_class
        return form_list
    ...
    def get_context_data(self, form, **kwargs):
        ...
        step_index = (
            int(self.steps.current.split("-")[-1])
            if "-" in self.steps.current
            else int(self.steps.current)
        )
        context["event_module"] = event_modules[step_index]
        
        # Add breadcrumbs to the context
        current_step_number = int(self.steps.current.split("-")[-1]) if "-" in self.steps.current else int(self.steps.current)
        step_name = event_modules[current_step_number].header if current_step_number < len(event_modules) else "Abschluss"

        context['breadcrumbs'] = [
            {"name": "Alle Veranstaltungen", "url": reverse("event-list")},
            {"name": event.name, "url": reverse("event-detail-overview", args=[event.slug])},
            {"name": "Anmeldung erstellen", "url": "#"},
            {"name": f"Schritt {current_step_number + 1}: {step_name}", "url": "#"},
        ]
        return context
        ...
        for i, form in enumerate(form_list):
            if i < len(event_modules):
                ...
                if obj.get("first_name", None):
                    ...
                    if obj.get("zip_code"):
                        try:
                            zip_code_instance = basic_models.ZipCode.objects.get(
                ...
                for field_name, value in form_data.items():
                    ...
                    if field_name.startswith("attribute_"):
                        ...
                        if attribute_module.field_type == "BoA":
                            ...
                            if value is not None:
                                kwargs["date_field"] = value
                            DateAttribute.objects.create(**kwargs)
                        elif attribute_module.field_type == "HtA":
                            # Handle HTML attribute
                            html_value = "" if value is None else value
                            HTMLAttribute.objects.create(
                                registration=registration,
                                attribute_module=attribute_module,
                                html_field=html_value,
                            )
                        elif attribute_module.field_type == "RaA":
                            if value:
                                try:
                                    option = AttributeChoiceOption.objects.get(
                                        text=value
                                    )
                                    RadioAttribute.objects.create(
                                        registration=registration,
                                        attribute_module=attribute_module,
                                        selected_option=option,
                                    )
                                except AttributeChoiceOption.DoesNotExist:
                                    pass
                        elif attribute_module.field_type == "MuA":
                            # Handle multi-select attribute
                            if value:
                                multi_attr = MultiSelectAttribute.objects.create(
                                    registration=registration,
                                    attribute_module=attribute_module,
                                )
                                    ...
                                    for option_id in value:
                                        try:
                                            if isinstance(option_id, int):
                                                option = (
                                                    AttributeChoiceOption.objects.get(
                                                        id=option_id
                                                    )
                                                )
                                            ...
                                            continue
                                    multi_attr.selected_options.set(options)
                        elif attribute_module.field_type == "ZiA":
                            # Handle zip code attribute
                            zip_value = "" if value is None else str(value)
                            try:
                                zip_code_instance = basic_models.ZipCode.objects.get(
                                    zip_code=zip_value
                                )
                            except basic_models.ZipCode.DoesNotExist:
                                zip_code_instance = None

                            ZipCodeAttribute.objects.create(
                                registration=registration,
                                attribute_module=attribute_module,
                                zip_code_field=zip_code_instance,
                            )
                        elif attribute_module.field_type == "EmA":
                            # Handle email attribute
                            email_value = "" if value is None else str(value)
                            EmailAttribute.objects.create(
                                registration=registration,
                                attribute_module=attribute_module,
                            ...
                            phone_value = "" if value is None else str(value)
                            PhoneAttribute.objects.create(
                                registration=registration,
                                attribute_module=attribute_module,
                                phone_number_field=phone_value,
                            )
                        elif attribute_module.field_type == "ScA":
                            # Handle scout group attribute
                            scout_group_value = "" if value is None else str(value)
                            ScoutGroupAttribute.objects.create(
                                registration=registration,
                                attribute_module=attribute_module,
                                scout_group=scout_group_value,
                            )
...
def download_registrations(request, reg_id):
    ...
    if (
        ...
        messages.error(
            request, "Du hast keine Berechtigung, diese Daten herunterzuladen."
        )
        return redirect("reg-detail-overview", reg_id=reg_id)

    response = HttpResponse(content_type="text/csv")
    response["Content-Disposition"] = (
        f'attachment; filename="{event.name}_registrations.csv"'
    )
    ...
    writer.writerow(
        [
            registration.id,
            (
                registration.scout_organisation.name
...
def download_participants(request, reg_id):
    ...
    response = HttpResponse(content_type="text/csv")
    response["Content-Disposition"] = (
        f'attachment; filename="{event.name}_participants.csv"'
    )
    ...
    for participant in registration.participants.all():
        writer.writerow(
            [
                ...
                (
                    participant.birthday.strftime("%Y-%m-%d")
...
def download_invoice(request, reg_id):
    ...
    if (
        ...
        messages.error(
            request, "Du hast keine Berechtigung, diese Rechnung herunterzuladen."
        )
        return redirect("reg-detail-overview", reg_id=reg_id)
    ...
    elements.append(
        Paragraph(
            f"Organisation: {registration.scout_organisation.name if registration.scout_organisation else 'N/A'}",
            normal_style,
        )
    )
    elements.append(
        Paragraph(
            f"Erstellt am: {registration.created_at.strftime('%d.%m.%Y')}", normal_style
        )
    )
    ...
    for p in registration.participants.all():
        ...
        if p.birthday:
            ...
            age = (
                today.year
                - p.birthday.year
                - ((today.month, today.day) < (p.birthday.month, p.birthday.day))
            )
            age = str(age)
    ...
    if len(participant_data) > 1:  # Only if we have participants
        participant_table = Table(
            participant_data, colWidths=[2 * inch, 1.5 * inch, 0.75 * inch, 1.25 * inch]
        )
    ...
    cost_data = [
        ...
        [
            ...
            f"{cost_per_participant} €",
            f"{total_cost} €",
        ],
    ]

    cost_table = Table(
        cost_data, colWidths=[2.5 * inch, 1 * inch, 1.5 * inch, 1.5 * inch]
    )
    cost_table.setStyle(
        TableStyle(
            [
    ...
    elements.append(
        Paragraph(
            f"Gesamtbetrag: {total_cost} €",
            ParagraphStyle(
                "Total", fontSize=14, alignment=2, fontName="Helvetica-Bold"
            ),
        )
    )
    ...
    elements.append(
        Paragraph(
            f"Bitte überweisen Sie den Gesamtbetrag bis zum {(event.registration_deadline or event.start_date).strftime('%d.%m.%Y') if (event.registration_deadline or event.start_date) else 'Veranstaltungsbeginn'} auf folgendes Konto:",
            normal_style,
        )
    )
    ...
    elements.append(
        Paragraph(
            f"Verwendungszweck: {event.name} - Anmeldung {registration.id}",
            normal_style,
        )
    )
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="rechnung_{event.name}_{registration.id}.pdf"'
    )
