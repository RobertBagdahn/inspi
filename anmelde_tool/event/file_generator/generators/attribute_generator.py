...
from anmelde_tool.event.file_generator.generators.abstract_generator import (
    AbstractGenerator,
)
from anmelde_tool.event.file_generator.generators.helper import (
    get_participants_by_registration,
)
...
class AttributeGenerator(AbstractGenerator):
    ...
    def generate(self) -> Workbook:
        ...
        registrations: QuerySet[event_models.Registration] = helper.get_registrations(
            event
        )
        ...
        booking_options: QuerySet[event_models.BookingOption] = (
            event_models.BookingOption.objects.filter(event=event)
        )
        ...
        count_tent_small: int = (
            attributes.filter(template_id=17).aggregate(
                count=Sum("integerattribute__integer_field")
            )["count"]
            or 0
        )
        count_tent_large: int = (
            attributes.filter(template_id=19).aggregate(
                count=Sum("integerattribute__integer_field")
            )["count"]
            or 0
        )
        ...
        medium_style = openpyxl.worksheet.table.TableStyleInfo(
            name="TableStyleMedium2", showRowStripes=True
        )
        table = openpyxl.worksheet.table.Table(
            ref=ref, displayName="Attribute", tableStyleInfo=medium_style
        )
