...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not UserModel.objects.filter(username="user").exists():
            user = UserModel.objects.create_user(
                "user", password="user", email="user@user.de", scout_display_name="user"
            )
        ...
        if not UserModel.objects.filter(username="mats").exists():
            user = UserModel.objects.create_user(
                "mats", password="mats", email="mats@mats.de", scout_display_name="Mats"
            )
